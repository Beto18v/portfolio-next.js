      ...
      <div className="relative z-[2] mx-auto w-full max-w-6xl">
        <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-12 lg:gap-6">
          ...
          >
            ...
            <motion.div className="flex flex-wrap items-center gap-3" {...enter(0.45)}>
            ...
            <motion.div
              className="mt-4"
              style={
                !shouldReduceMotion ? { y: titleY, opacity: titleOpacity } : undefined
              }
            >
              <SplitTextTitle
                text={title}
            ...
            <motion.div className="mt-5 flex flex-wrap items-center gap-3" {...enter(0.65)}>
              <MagneticButton>
                <motion.span
                ...
                >
                  <Button asChild size="lg" className="h-11 rounded-full px-7">
                    <Link href={siteConfig.hero.primaryCTA.href} aria-label={primaryLabel}>
                      {primaryLabel}
                    </Link>
                  </Button>
                </motion.span>
            ...
            {/* Proof strip */}
            <motion.div className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-2" {...enter(0.7)}>
              {proof.map((item, i) => (
                <span
                  key={item.en}
                ...
                >
                  ...
                  {item[locale]}
                  {i < proof.length - 1 && (
                    <span className="ml-1 h-1 w-1 shrink-0 rounded-full bg-border" aria-hidden="true" />
                  )}
                </span>
              ))}
            </motion.div>
